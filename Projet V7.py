...
def init_game():
    global paddle_x, paddle_y, ball_x, ball_y, ball_dx, ball_dy, bricks, game_over, game_won, lose_sound_played
    ...
    for row in bricks:
        for brick in row:
            if brick.colliderect(ball_rect):
                ...
                ball_dy *= -1
                row.remove(brick)
                # Vérifiez immédiatement après la suppression d'une brique
                if all(not row for row in bricks):  # Check if all rows are empty
                    game_won = True
                    if win_sound:
                        win_sound.play()
                return
...
def draw_game_over():
    ...
    restart_text = small_font.render("Press R to Restart", True, ORANGE)
...
def draw_game_won():
    ...
    restart_text = small_font.render("Press R to Restart", True, GREEN)
    screen.blit(restart_text, (screen_width // 2 - restart_text.get_width() // 2,
